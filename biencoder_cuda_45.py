...
parser.add_argument('--context-bsz', type=int, default=4)#4
parser.add_argument('--gloss-bsz', type=int, default=256)#256
parser.add_argument('--encoder-name', type=str, default='bert-base',
    choices=['bert-base', 'bert-large', 'roberta-base', 'roberta-large', 'distilbert'])
...
def _train(train_data, model, gloss_dict, optim, schedule, criterion, gloss_bsz=-1, max_grad_norm=1.0, multigpu=False, silent=False, train_steps=-1):
    ...
    for i, (context_ids, context_attn_mask, context_output_mask, example_keys, _, labels) in train_data:
        ...
        for j, (key, label) in enumerate(zip(example_keys, labels)):
            ...
            output = torch.div(output, torch.norm(output))
...
def train_model(args):
    ...
    if args.balanced:
        for key in train_gloss_dict:
            ...
            criterion[key] = torch.nn.CrossEntropyLoss(reduction='mean', weight=train_gloss_weights[key])
            #criterion[key] = torch.nn.KLDivLoss(reduction='none', weight=train_gloss_weights[key])
            #criterion[key] = torch.nn.MultiLabelSoftMarginLoss(reduction='none', weight=train_gloss_weights[key])
    else:
        for key in train_gloss_dict:
            #criterion[key] = torch.nn.CrossEntropyLoss(reduction='none')
            criterion[key] = torch.nn.CrossEntropyLoss(reduction='mean')
            #print(f'criterion key:{key}\n{criterion[key]}')
            #criterion[key] = torch.nn.KLDivLoss(reduction='none')
            #criterion[key] = torch.nn.MultiLabelSoftMarginLoss(reduction='none')
    ...
    #schedule = WarmupCosineSchedule(optimizer, warmup_steps=args.warmup, t_total=t_total)
